...
import { type Tailwind, type Variant, VariantEnum } from './types';
...
export type { Tailwind, Variant, StyleXProps };
...
export const useStyleX = <T extends Record<string, any>>(
  xStyles: T,
  params: UseStyleXParams = {},
): {
  classes: Classes<T>;
} => {
  const { isWithAttrs = false, isHash = false, tailwindStyles, theme } = params;
