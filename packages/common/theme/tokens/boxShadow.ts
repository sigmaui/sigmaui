export const boxShadows = {
  xs: '0px 1px 2px 0px rgba(41, 43, 51, 0.05)',
  md: '0px 4px 8px -2px rgba(41, 43, 51, 0.1), 0px 2px 4px -2px rgba(41, 43, 51, 0.05)',
  lg: '0px 12px 16px -4px rgba(41, 43, 51, 0.1), 0px 4px 6px -2px rgba(41, 43, 51, 0.05)',
  xl: '0px 20px 24px -4px rgba(41, 43, 51, 0.1), 0px 8px 8px -4px rgba(41, 43, 51, 0.05)',
  '2xl': '0px 24px 48px -12px rgba(41, 43, 51, 0.1)',
  '3xl': '0px 32px 64px -12px rgba(41, 43, 51, 0.1)',
  focused: '0 0 0 2px var(--stroke-brand-weak, rgba(247, 127, 0, 0.1))'
} as const
