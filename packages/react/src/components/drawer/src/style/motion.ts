...
const genMaskMotionStyle = ({
  ...
  styles[
    `& ${componentCls}-mask-motion-enter, & ${componentCls}-mask-motion-appear`
  ] = { opacity: 0 };
  styles[
...
const genPanelMotionStyle = ({
  ...
  ['right', 'left', 'top', 'bottom'].forEach((dir) => {
    ...
    styles[
      `& ${prefix}-enter-active, & ${prefix}-appear-active, & ${prefix}-leave-active`
    ] = {
      transition: `all ${motionDuration}`,
    };
